...
The Kawra Core daemon can be configured to act as a trusted "border
router", implementing the kawra wire protocol and relay, making
...
The ZeroMQ feature in Kawra Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building kawrad:
...
For instance:
    ...
    $ kawrad -zmqpubhashtx=tcp://127.0.0.1:223917 \
               -zmqpubhashtx=tcp://192.168.1.2:223917 \
               -zmqpubhashblock="tcp://[::1]:223918" \
               -zmqpubrawtx=ipc:///tmp/kawrad.tx.raw \
               -zmqpubhashtxhwm=10000

Each PUB notification has a topic and body, where the header
corresponds to the notification type. For instance, for the
notification `-zmqpubhashtx` the topic is `hashtx` (no null
terminator). These options can also be provided in kawra.conf.
...
From the perspective of kawrad, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into kawrad directly. Furthermore, no information is
...
using. Kawrad appends an up-counting sequence number to each
