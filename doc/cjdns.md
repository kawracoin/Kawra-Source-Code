# CJDNS support in Kawra Core

It is possible to run Kawra Core over CJDNS, an encrypted IPv6 network that
...
done outside of Kawra Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Kawra network and individual nodes.
...
before it will work with your Kawra Core node. This is described in steps
...
## Run Kawra Core with CJDNS

Once you are connected to the CJDNS network, the following Kawra Core
...
When enabled, this option tells Kawra Core that it is running in an
...
IPv6 local network. This helps Kawra Core perform better address management:
...
CJDNS support was added to Kawra Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `kawra-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/kawra/kawra/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Kawra Core

There are several ways to see your CJDNS address in Kawra Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `kawra-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `kawra-cli getpeerinfo`).
