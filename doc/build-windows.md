...
Below are some notes on how to build Kawra Core for Windows.

The options known to work for building Kawra Core on Windows are:
...
Note that for WSL the Kawra Core source path MUST be somewhere in the default mount file system, for
example /usr/src/kawra, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\kawra`, for example:

    make install DESTDIR=/mnt/c/workspace/kawra
