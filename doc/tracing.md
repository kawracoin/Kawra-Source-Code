# User-space, Statically Defined Tracing (USDT) for Kawra Core

Kawra Core includes statically defined tracepoints to allow for more
...
**Note**: Kawra Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Kawra Core
...
Multiple tools can list the available tracepoints in a `kawrad` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Kawra Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/kawrad
stap net        outbound_message 0x0000000000107c05 /src/kawrad
stap validation block_connected  0x00000000002fb10c /src/kawrad
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Kawra Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/kawrad | grep NT_STAPSDT -A 4 -B 2
