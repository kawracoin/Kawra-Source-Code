# PSBT Howto for Kawra Core

Since Kawra Core 0.17, an RPC interface exists for Partially Signed Kawra
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/kawra/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Kawra transactions that are not fully signed
...
#### Multisig with multiple Kawra Core instances
...
Kawra Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `kawra-cli` is used.
...
Later, when *V* KAWRA has been received on *Amulti*, and Bob and Carol want to
