...
/** Call node.updateRwSetting() with Kawra 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Kawra 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/kawra/kawra/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because kawra 22.x will not try to read these.
...
//! Convert enabled/size values to kawra -prune setting.
...
//! Get pruning enabled value to show in GUI from kawra -prune setting.
...
//! Get pruning size value to show in GUI from kawra -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayKawraUnit")) {
        settings.setValue("DisplayKawraUnit", QVariant::fromValue(KawraUnit::KAWRA));
    }
    QVariant unit = settings.value("DisplayKawraUnit");
    if (unit.canConvert<KawraUnit>()) {
        m_display_kawra_unit = unit.value<KawraUnit>();
    } else {
        m_display_kawra_unit = KawraUnit::KAWRA;
        settings.setValue("DisplayKawraUnit", QVariant::fromValue(m_display_kawra_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if kawra.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_kawra_unit);
...
{
    if (new_unit.isNull() || new_unit.value<KawraUnit>() == m_display_kawra_unit) return;
    m_display_kawra_unit = new_unit.value<KawraUnit>();
    QSettings settings;
    settings.setValue("DisplayKawraUnit", QVariant::fromValue(m_display_kawra_unit));
    Q_EMIT displayUnitChanged(m_display_kawra_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/kawra/kawra/pull/8273
    ...
    // (https://github.com/kawra-core/gui/issues/567).
