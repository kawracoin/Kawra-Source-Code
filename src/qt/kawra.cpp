...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<KawraUnit>("KawraUnit");
#else
    qRegisterMetaType<KawraUnit>("KawraUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. kawra_de.qm (shortcut "de" needs to be defined in kawra.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. kawra_de_DE.qm (shortcut "de_DE" needs to be defined in kawra.qrc)
...
static const char* qt_argv = "kawra-qt";

KawraApplication::KawraApplication()
...
void KawraApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the KawraApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", KawraGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void KawraApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool KawraApplication::createOptionsModel(bool resetSettings)
{
...
void KawraApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new KawraGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &KawraGUI::quitRequested, this, &KawraApplication::requestShutdown);
...
void KawraApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void KawraApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool KawraApplication::baseInitialize()
{
    return node().baseInitialize();
}

void KawraApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &KawraApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &KawraApplication::handleRunawayException);
    connect(this, &KawraApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &KawraApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void KawraApplication::parameterSetup()
{
...
void KawraApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void KawraApplication::requestInitialize()
{
...
void KawraApplication::requestShutdown()
{
...
void KawraApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // kawra: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &KawraGUI::handlePaymentRequest);
            connect(window, &KawraGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void KawraApplication::handleRunawayException(const QString &message)
{
...
void KawraApplication::handleNonFatalException(const QString& message)
{
...
WId KawraApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool KawraApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", KawraGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(kawra);
    Q_INIT_RESOURCE(kawra_locale);
    ...
    /// 6-7. Parse kawra.conf, determine network, switch to network specific
    ...
    // kawra: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
