...
#include <qt/kawragui.h>

#include <qt/kawraunits.h>
...
const std::string KawraGUI::DEFAULT_UIPLATFORM =
...
KawraGUI::KawraGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &KawraGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &KawraGUI::showModalOverlay);
...
void KawraGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Kawra address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and kawra: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &KawraGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &KawraGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &KawraGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Kawra addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Kawra addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Kawra Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Kawra Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a kawra: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Kawra command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &KawraGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &KawraGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &KawraGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &KawraGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &KawraGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &KawraGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &KawraGUI::setCurrentWallet, Qt::QueuedConnection);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &KawraGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &KawraGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &KawraGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &KawraGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &KawraGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &KawraGUI::showDebugWindow);
}

void KawraGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void KawraGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &KawraGUI::setCurrentWalletBySelectorIndex);
...
void KawraGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &KawraGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &KawraGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &KawraGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &KawraGUI::showProgress);
...
#ifdef ENABLE_WALLET
void KawraGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void KawraGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &KawraGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &KawraGUI::removeWallet);
...
WalletController* KawraGUI::getWalletController()
{
    return m_wallet_controller;
}

void KawraGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &KawraGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &KawraGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &KawraGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &KawraGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &KawraGUI::incomingTransaction);
    connect(this, &KawraGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void KawraGUI::removeWallet(WalletModel* walletModel)
{
...
void KawraGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void KawraGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void KawraGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void KawraGUI::setWalletActionsEnabled(bool enabled)
{
...
void KawraGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &KawraGUI::toggleHidden);
...
void KawraGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void KawraGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void KawraGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void KawraGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void KawraGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void KawraGUI::openClicked()
{
...
void KawraGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void KawraGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void KawraGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void KawraGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void KawraGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void KawraGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void KawraGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void KawraGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Kawra network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void KawraGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void KawraGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void KawraGUI::updateHeadersSyncProgressLabel()
{
...
void KawraGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void KawraGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &KawraGUI::quitRequested);
...
void KawraGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void KawraGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void KawraGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &KawraGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &KawraGUI::show);
                e->ignore();
            }
        }
    }
...
void KawraGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void KawraGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void KawraGUI::incomingTransaction(const QString& date, KawraUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(KawraUnits::formatWithUnit(unit, amount, true));
...
void KawraGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void KawraGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool KawraGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool KawraGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void KawraGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void KawraGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void KawraGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void KawraGUI::updateWindowTitle()
{
...
void KawraGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void KawraGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void KawraGUI::detectShutdown()
{
...
void KawraGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void KawraGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(KawraGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = KawraGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void KawraGUI::subscribeToCoreSignals()
{
...
void KawraGUI::unsubscribeFromCoreSignals()
{
...
bool KawraGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<KawraUnit> units = KawraUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const KawraUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, KawraUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const KawraUnit u : KawraUnits::availableUnits()) {
        menu->addAction(KawraUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(KawraUnit newUnits)
{
    setText(KawraUnits::longName(newUnits));
}
