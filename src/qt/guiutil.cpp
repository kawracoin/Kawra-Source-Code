...
#include <qt/kawraaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Kawra address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new KawraAddressEntryValidator(parent));
    widget->setCheckValidator(new KawraAddressCheckValidator(parent));
}
...
bool parseKawraURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no kawra: URI
    if(!uri.isValid() || uri.scheme() != QString("kawra"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!KawraUnits::parse(KawraUnit::KAWRA, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseKawraURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseKawraURI(uriInstance, out);
}

QString formatKawraURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("kawra:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(KawraUnits::format(KawraUnit::KAWRA, info.amount, false, KawraUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openKawraConf()
{
    ...
    /* Open kawra.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Kawra.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Kawra (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Kawra (%s).lnk", chain));
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "kawra.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("kawra-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a kawra.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Kawra\n";
        else
            optionFile << strprintf("Name=Kawra (%s)\n", chain);
