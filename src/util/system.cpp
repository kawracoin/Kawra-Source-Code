...
const char * const KAWRA_CONF_FILENAME = "kawra.conf";
const char * const KAWRA_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //kawra-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", KAWRA_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Kawra
    // macOS: ~/Library/Application Support/Kawra
    // Unix-like: ~/.kawra
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Kawra";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Kawra";
#else
    // Unix-like
    return pathRet / ".kawra";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", KAWRA_CONF_FILENAME));
}
