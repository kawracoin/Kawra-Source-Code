...
TMPDIR_PREFIX = "kawra_func_test_"
...
class KawraTestMetaClass(type):
    """Metaclass for KawraTestFramework.

    Ensures that any attempt to register a subclass of `KawraTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'KawraTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("KawraTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("KawraTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class KawraTestFramework(metaclass=KawraTestMetaClass):
    """Base class for a kawra test script.

    Individual kawra test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave kawrads and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop kawrads after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use kawra-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_kawrad = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "kawrad" + config["environment"]["EXEEXT"],
        )
        fname_kawracli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "kawra-cli" + config["environment"]["EXEEXT"],
        )
        fname_kawrautil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "kawra-util" + config["environment"]["EXEEXT"],
        )
        self.options.kawrad = os.getenv("KAWRAD", default=fname_kawrad)
        self.options.kawracli = os.getenv("KAWRACLI", default=fname_kawracli)
        self.options.kawrautil = os.getenv("KAWRAUTIL", default=fname_kawrautil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: kawrads were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("KawraRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'kawrad', self.options.kawrad) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'kawra-cli', self.options.kawracli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a kawrad test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple kawrad test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as kawrad's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    kawrad=self.options.kawrad,
                    kawra_cli=self.options.kawracli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in kawra.conf
    ...
    def skip_if_no_kawrad_tracepoints(self):
        """Skip the running test if kawrad has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("kawrad has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_kawrad_zmq(self):
        """Skip the running test if kawrad has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("kawrad has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if kawra-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("kawra-wallet has not been compiled")

    def skip_if_no_kawra_util(self):
        """Skip the running test if kawra-util has not been compiled."""
        if not self.is_kawra_util_compiled():
            raise SkipTest("kawra-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if kawra-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("kawra-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether kawra-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether kawra-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_kawra_util_compiled(self):
        """Checks whether kawra-util was compiled."""
        return self.config["components"].getboolean("ENABLE_KAWRA_UTIL")
