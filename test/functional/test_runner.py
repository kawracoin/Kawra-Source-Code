...
`test/functional/test_framework/test_framework.py:KawraTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_kawra_cli.py --legacy-wallet',
    'interface_kawra_cli.py --descriptors',
...
def main():
    ...
    enable_kawrad = config["components"].getboolean("ENABLE_KAWRAD")

    if not enable_kawrad:
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    ...
    # Warn if kawrad is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "kawrad"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a kawrad process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
...
class RPCCoverage():
    ...
    commands per `kawra-cli help` (`rpc_interface.txt`).
