...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing kawra-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='kawra-cli', help='kawra-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Kawra address to which the faucet should send')
...
parser.add_argument('kawra_cli_args', nargs='*', help='Arguments to pass on to kawra-cli (default: -signet)')

args = parser.parse_args()

if args.kawra_cli_args == []:
    args.kawra_cli_args = ['-signet']


def kawra_cli(rpc_command_and_params):
    argv = [args.cmd] + args.kawra_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = kawra_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = kawra_cli(['getnewaddress', 'faucet', 'bech32'])
