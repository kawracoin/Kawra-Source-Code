...
""" Example logging Kawra Core mempool events using the mempool:added,
...
def main(kawrad_path):
    kawrad_with_usdts = USDT(path=str(kawrad_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    kawrad_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    kawrad_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    kawrad_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    kawrad_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[kawrad_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/kawrad")
        exit(1)
