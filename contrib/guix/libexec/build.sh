...
(
    ...
    case "$HOST" in
        *mingw*)
            make deploy ${V:+V=1} KAWRA_WIN_INSTALLER="${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our Kawra Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Kawra Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        # copy over the example kawra.conf file. if contrib/devtools/gen-kawra-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/kawra.conf" "${DISTNAME}/"
