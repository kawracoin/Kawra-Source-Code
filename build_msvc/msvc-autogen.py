...
def parse_config_into_kawra_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Kawra Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/kawra_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/kawra_config.h'), "w", encoding="utf8") as kawra_config:
        kawra_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='Kawra-core msbuild configuration initialiser.')
    ...
    parse_config_into_kawra_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/kawra_config.h'), os.path.join(SOURCE_DIR, 'config/kawra-config.h'))
